...
model Sink_pT
  //Sink model, required to specify the temperature and pressure
  //Flow medium
...
equation
  if size(X,1) > 0 then
    FluidIn = Medium.setState_pTX(p_su, T_su, X);
  else 
    FluidIn = Medium.setState_pTX(p_su, T_su, Xnom);
  end if;
  h_su = Medium.specificEnthalpy(FluidIn);
